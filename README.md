...
- 所有命令收到后都会对原消息贴一个表情回应，用于快速判断插件是否已响应。
  插件会从 `message_obj.message_id` 读取消息 ID 并转成整数，再通过事件自带的
  `self_id` 绑定对应的 NapCat 账号，调用 `set_msg_emoji_like`，确保表情贴在原命令消息上。
...
- 执行命令时不再发送过程提示，最终只返回一条合成消息：图片在上，参数与标签在下。
- 生成结果参数包含尺寸、耗时、正向/反向 Prompt、Seed、步数、分辨率、采样器、调度器、CFG比例、强度、生成器、模型、文件大小、标签数、NSFW、可见性与作品页。
- `/生图`、`/画图`、`/搜图` 的最终返回都带参数与标签；`/搜图` 通过解析原图嵌入的
  ComfyUI 工作流，展示作品实际使用的完整正向/反向 Prompt，以及模型、CFG比例、强度、
  采样器、调度器、步数、Seed、文件大小等。
