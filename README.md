...
- /roll：按 Danbooru 标签随机挑选一张已公开的安全 AI 图片，不消耗生成配额。
...
`/roll` 同样支持 `--size`、`--source`、`--model`（不支持 `--rank`，排名由插件随机决定）。
...
- 生成接口每周有独立配额，只有成功出图才消耗；`/搜图`、`/roll` 不消耗配额。
...
- `/画图` 与 `/生图` 可生成 NSFW 内容（需用户明确要求），结果保持私密并由站方打 NSFW 标记；`/搜图` 与 `/roll` 只会返回公开安全图片。
